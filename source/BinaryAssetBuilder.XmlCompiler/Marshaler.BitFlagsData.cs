...
public static partial class Marshaler
{
    ...
    public static unsafe void Marshal(string text, MGPowerPrereqBitFlags* objT, Tracker state)
    {
        string[] tokens = text.Split(WhiteSpaces, System.StringSplitOptions.RemoveEmptyEntries);
        if (tokens.Length == 0)
        {
        ...
        for (int idy = 0; idy < tokens.Length; ++idy)
        {
            ...
            if (string.Equals(token, "ALL", System.StringComparison.Ordinal))
            {
                for (int idx = 0; idx < MGPowerPrereqBitFlags.NumSpans; ++idx)
                {
                    objT->Value[idx] = uint.MaxValue;
                }
                continue;
            }
            MGPowerPrereqFlagType value = (MGPowerPrereqFlagType)(-1);
            Marshal(token, &value, state);
            if (value != (MGPowerPrereqFlagType)(-1))
            {
                uint uintValue = (uint)value;
                if (uintValue < MGPowerPrereqBitFlags.Count)
                {
                    if (includeToken)
                    {
                        objT->Value[uintValue / MGPowerPrereqBitFlags.BitsInSpan] |= (uint)(1 << (int)(uintValue % MGPowerPrereqBitFlags.BitsInSpan));
                    }
                    else
                    {
                        objT->Value[uintValue / MGPowerPrereqBitFlags.BitsInSpan] ^= (uint)(1 << (int)(uintValue % MGPowerPrereqBitFlags.BitsInSpan));
    ...
    public static unsafe void Marshal(Value value, MGPowerPrereqBitFlags* objT, Tracker state)
    {
        if (value is null)
        {
            return;
